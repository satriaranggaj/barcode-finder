...
@if (($pendingReferences ?? 0) > 0)
    <div class="mb-6 rounded-xl border border-[#ffc20e] bg-[#fff8d6] px-4 py-3 text-sm font-semibold text-[#543019]">{{ $pendingReferences }} konfirmasi terverifikasi menunggu masuk index — jalankan <span class="font-mono">search:build-index --include-verified</span> lalu restart AI worker agar AI semakin hafal barang.</div>
@endif
...
<section>
    ...
    <div class="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        @foreach ($productsWithPhotosList as $product)
            <a href="{{ route('admin.products.show', $product) }}" class="group overflow-hidden rounded-2xl border border-[#eadfca] bg-[#fffdf4] transition duration-300 hover:-translate-y-1 hover:border-[#ffc20e] hover:shadow-[5px_5px_0_#fff200]"><div class="aspect-square overflow-hidden bg-[#f5e6bd]"><img src="{{ $product->photos->first()->url }}" alt="{{ $product->description }}" class="h-full w-full object-cover transition duration-500 group-hover:scale-105"></div><div class="p-4"><div class="flex items-center justify-between gap-3"><p class="font-mono text-sm font-bold text-[#8b5e00]">{{ $product->sku }}</p><span class="rounded-full bg-[#fff8d6] px-2 py-1 text-[10px] font-bold text-[#8b5e00]">{{ $product->photos->count() }} design</span></div><p class="mt-2 line-clamp-2 text-sm font-semibold leading-5 text-[#543019]">{{ $product->description ?: 'Deskripsi belum tersedia' }}</p><p class="mt-3 text-xs font-bold text-[#8b5e00]">Edit item →</p></div></a>
        @endforeach
    </div>
