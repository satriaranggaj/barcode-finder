...
<div class="mb-7">
    ...
    <div class="mt-4 rounded-2xl p-3 sm:p-4">
        <x-object-selection form="home-image-search-form" />
        ...
        <div class="mt-3 grid items-start gap-3 sm:grid-cols-[minmax(190px,0.8fr)_minmax(280px,1.2fr)]">
            <div>
                <form
                ...
                >
                    ...
                    >
                    @auth
                        @if (config('retrieval.feedback_enabled') && config('retrieval.driver') === 'faiss')
                            <label class="mt-4 flex gap-2 text-sm"><input type="checkbox" name="allow_feedback" value="1"> Izinkan konfirmasi SKU setelah pencarian. Foto disimpan sementara selama 15 menit; hanya foto yang saya konfirmasi disimpan sebagai data terverifikasi.</label>
                        @endif
                    @endauth
                </form>
            </div>
            ...
            <a href="{{ route('products.show', $product) }}" class="group overflow-hidden rounded-2xl border border-[#eadfca] bg-[#fffdf4] transition duration-300 hover:-translate-y-1 hover:border-[#ffc20e] hover:shadow-[5px_5px_0_#fff200]">
                <div class="aspect-square overflow-hidden bg-[#f5e6bd]">
                    @if ($product->photos->first())
                        <img src="{{ $product->photos->first()->thumbnail_url }}" alt="{{ $product->description }}" loading="lazy" class="h-full w-full object-cover transition duration-500 group-hover:scale-105">
                    @else
                        <div class="flex h-full flex-col items-center justify-center text-[#8b7555]"><span class="text-4xl">▧</span><span class="mt-3 text-xs font-bold uppercase tracking-wider">Foto belum tersedia</span></div>
                    @endif
                </div>
